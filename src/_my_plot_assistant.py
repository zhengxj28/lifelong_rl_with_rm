...
def plot_eval_transfer(title, data_name, alg_list, plot_task_index, to_steps=True,save_fig=True,
    ...
    for alg_name in alg_list:
        if use_normalize: plot_list.append(np.load(directory + data_name + alg_name + "norm.npy"))
        else: plot_list.append(np.load(directory + data_name + alg_name + ".npy"))
    # plot_list[i].shape=[len(tasks),repeated_test_times,steps_num]
    color_list = ["blue", "purple", "red", "yellow", "green", "hotpink"]
    # plt.clf()
    fig, ax = plt.subplots(1, 1, figsize=(6, 4))
    plt.title(title)
    if to_steps: plt.ylabel("Steps to Complete Task")
    else: plt.ylabel("Cumulative Rewards")
...
def plot_op_laws(title, data_name,  data_index,
    ...
    for curve_i in range(2+len(plot_dict)):
        if curve_i==0: plot_i=plot_qrm[1] # 1 is the target task
        elif curve_i==1: plot_i=plot_qrmrs[1]
        else: plot_i=plot_dict[data_index[curve_i-2]][1]
