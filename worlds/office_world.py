...
class OfficeWorld:
    ...
    def get_state(self):  # added by myself
        x, y = self.agent
        N, M = 12, 9
        return int(x*M+y)
    ...
    def _load_map(self):
        ...
        self.objects[(4, 5)] = "f"  # COFFEE
        # self.objects[(5, 5)] = "f"  # COFFEE
        ##################################
    ...
    def initialize(self,random_init=False):  # added by myself
        if random_init:
...
def play():
    ...
    # play the game!
    # tasks = ["../../Maps/office/reward_machines/t%d.txt"%i for i in [1,2,3,4]]
    tasks = [r"E:\Machine Learning\RL implementation\Reward Machines\Operations on Reward Machines\experiments\office\reward_machines\t%d.txt"%i for i in [1,2,3,4]]
    reward_machines = []
    for t in tasks:
        reward_machines.append(RewardMachine(t,False, 0.9))
